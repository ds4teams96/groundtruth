...
def pre_filter_items_resuelve(list_files, path_files='', b_debug=False):
...
def generate_index_file_resuelve(option_list, q_sublevels=5, n_level_id=[], style_level_id=[], chapter=1, b_debug=False):
    ...
    if q_sublevels!=0:
                    ...
                    if len(jump_list)==0 or (n_level_id[0]==0 and arr_levels[0]==1): 
                        ...
                        if n_level_id[len(n_level_id)-q_sublevels]!=0:
                            ...
                            if jump_list[0][3] == 0: # se insertó como un posible capitulo
                                ...
                                tout_1, tout_2, tout_3 = generate_index_file_resuelve(jump_list[1:],q_sublevels, n_level_id, style_level_id, b_debug=b_debug)
                            else:
                                jump_list.append(line)
                                if b_debug: print(ident, "BP54 - niveles: ", n_level_id, arr_levels, jump_list, q_sublevels-1, n_level_id, style_level_id)
                                tout_1, tout_2, tout_3 = generate_index_file_resuelve(jump_list, q_sublevels-1, n_level_id, style_level_id, b_debug=b_debug)
        ...
        if len(jump_list)!=0 and n_level_id[len(n_level_id)-q_sublevels]!=0:
            ...
            if jump_list[0][3] == 0: # se insertó como un posible capitulo
                ...
                tout_1, tout_2, tout_3 = generate_index_file_resuelve(jump_list[1:], max(q_sublevels,len(n_level_id)), 
                                                             n_level_id, style_level_id, b_debug=b_debug)
                # arr_levels = tout_2
                if b_debug: print(ident, "BP62 - niveles: ", n_level_id, arr_levels)
            else:
                tout_1, tout_2, tout_3 = generate_index_file_resuelve(jump_list, q_sublevels-1, n_level_id, 
...
for file_name in list_files_index_ocr:
    index_items_ocr, _, _ = generate_index_file_resuelve([item for item in temp_list_index_items_ocr if item[0]==file_name], q_sublevels=n_max_sublevels, b_debug=False)
    save_index_file(index_items_ocr, engine, sType='Resuelve', n_level=n_max_sublevels)
